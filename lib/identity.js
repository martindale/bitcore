...
Identity.prototype.set = function(obj) {
  ...
  this.keypair    = this.keypair || null;
...
Identity.prototype.fromPrivkey = function( privkey , networkstr ) {
  var self = this;
  
  self.keypair = Keypair().fromPrivkey( privkey );
  
  self.hashbuf    = Hash.sha256ripemd160( self.keypair.toBuffer() );
  self.networkstr = networkstr || 'ephemeral';
  self.typestr    = 'identity';

  return self;
}

Identity.prototype.fromKeypair = function( keypair ) {
  var self = this;

  self.keypair = Keypair( keypair );

  self.networkstr = keypair.networkstr;
  self.typestr    = 'identity';
...
Identity.prototype.toPubkey = function() {
  var self = this;
  var pubkey = Pubkey().fromBuffer( self.hashbuf );
  return pubkey;
}
